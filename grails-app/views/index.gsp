<html>
    <head>
        <title>Whyteboard - A painting application</title>
        <p:css name="index" />
    </head>
    <body>
        <div id="about">
        	<h2>Whyteboard is...</h2>
        	<p>Whyteboard is a free &amp; open source painting application for Linux, Windows and Mac. It is suited towards creating visual presentations and for overlaying PDF images with annotations.</p>
        	<p>It is designed for Windows, Mac and Linux</p>      	 
        </div>
        
        <div id="download">
        	<h2>Download Whyteboard</h2>
			<p>The latest version of Whyteboard is <span id="latest-version">0.41.1</span></p>
			
			<ul id="platforms">
				<li><a href="#" id="download-windows" title="Download Whyteboard for Windows">Windows</a></li>
				<li><a href="#" id="download-linux" title="Download Whyteboard for Linux">Linux</a></li>
				<li><a href="#" id="download-mac" title="Download Whyteboard for Mac">Mac</a></li>
			</ul>
        </div>
        
        <div id="screenshots">
        	<h2>Screenshots</h2>
        	<div>
        	<img src="${g.resource(dir: 'images/screenshots', file: 'program-windows-small.png')}" alt="" />
        	<img src="${g.resource(dir: 'images/screenshots', file: 'program-linux-small.png')}" alt="" />
        	</div>
        </div>
    </body>
</html>